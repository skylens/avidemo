...
<html lang="en">
	...
	<body>
	  ...
	  <h1>Hello world!</h1>
	  <h3>My hostname is <font color="white">{{ .Hostname }}</font>, and the current version is <font color="white">v2</font></h3>
	  {{- range .IPv4s }}
	  <h3>IPv4 is <font color="white">{{ . }}</font></h3>
	  {{- end }}
	  {{- range .IPv6s }}
	  <h3>IPv6 is <font color="white">{{ . }}</font></h3>
	  {{- end }}
	  <h3>I'm running on host <font color="white"></font></h3>
	  <br />
